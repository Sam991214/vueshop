<template>
  <div>
    ...
    <el-card>
      ...
      <!-- 表格 -->
      <!-- :selection-type="false" :expand-type="false"移除复选框和展开 ，:selection-type="false" :expand-type="false"展示索引列-->
      <!-- border纵向边框，:show-row-hover="false"去除默认高亮选中效果 -->
      <tree-table class="treeTable" :data="catelist" :columns="columns" :selection-type="false" :expand-type="false" show-index index-text="#" border :show-row-hover="false">
        <!-- 是否有效 -->
        <!-- 自定义渲染用作用域插槽，type配置template属性，指定自定义插槽，名字自取，配置在slot上 -->
        <template slot="isok" slot-scope="scope">
          <!-- if-else区分有无效，cat_deleted区分有无效 -->
...
export default {
  data() {
    return {
      // 查询条件
      querInfo: {
        // 值：1，2，3 分别表示显示一层二层三层分类列表,默认·为三，显示所有三级
        type: 3,
        // 当前页码值
        pagenum: 1,
        // 每页显示多少条数据
      ...
      parentCateList: [],
      // 指定级联选择器的配置对象
      // id和value相关，label和分类名相关，children决定父子节点嵌套关系
      cascaderProps: {
        value: 'cat_id',
        label: 'cat_name',
        children: 'children'
  ...
  methods: {
    ...
    // 监听 pagesize 改变，拿最新pagesize，刷新数据列表
